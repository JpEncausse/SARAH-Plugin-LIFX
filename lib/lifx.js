...
Lifx.prototype.getBulbidByName = function(lifxName) {
	var addrToSearch = lifxName;
	if (typeof lifxName != 'string') {
		addrToSearch = lifxName.toString('hex');
	}
	
	for (var i in this.bulbs) {
		if (this.bulbs[i].name == addrToSearch) {
			return this.bulbs[i];
		}
	}
	return false;
...
// Reboot a bulb
Lifx.prototype.RebootBulb = function(bulb) {
	var message = packet.reboot();
	this._sendToOneOrAll(message, bulb);
};	

// Get Bulb FW Version
Lifx.prototype.BulbFirmware = function(bulb) {
	var message = packet.getMeshFirmware();
	this._sendToOneOrAll(message, bulb);
};

// Fct BulbTechInfo - Ingo technique du bulb (signal, tx, temp)
Lifx.prototype.BulbTechInfo = function(bulb) {
